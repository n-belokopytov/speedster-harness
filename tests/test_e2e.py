...
def _start_agent(role: str, port: int, ready_event: threading.Event) -> tuple[AgentServer, threading.Thread]:
    """Start an agent server in a background thread."""

    config = AgentServerConfig(role=role, model=f"vllm/{role}-e2e", host="127.0.0.1", port=port)
